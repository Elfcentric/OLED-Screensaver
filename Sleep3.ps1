...
# --- Setup for Screen 3 (Tertiary) ---
$screen3 = [System.Windows.Forms.Screen]::AllScreens[2]
$bounds3 = $screen3.Bounds
$overlay3 = New-Object Windows.Forms.Form
$overlay3.FormBorderStyle = 'None'
$overlay3.WindowState = 'Normal'
$overlay3.TopMost = $true
$overlay3.BackColor = 'Black'
$overlay3.Opacity = 1
$overlay3.StartPosition = 'Manual'
$overlay3.Location = $bounds3.Location
$overlay3.Size = $bounds3.Size
$overlay3.ShowInTaskbar = $false
$lastSeenOnScreen3 = Get-Date
...
while ($true) {
    ...
    # Check cursor position for Screen 3
    elseif ($cursorPos.X -ge $bounds3.X -and $cursorPos.X -lt ($bounds3.X + $bounds3.Width) -and
            $cursorPos.Y -ge $bounds3.Y -and $cursorPos.Y -lt ($bounds3.Y + $bounds3.Height)) {
        $lastSeenOnScreen3 = Get-Date
        if ($overlay3.Visible) {
            $overlay3.Hide()
        }
    ...
    # Check idle time for Screen 3
    if (([DateTime]::Now - $lastSeenOnScreen3).TotalMinutes -ge $idleTimeMinutes) {
        if (-not $overlay3.Visible) {
            $overlay3.Show()
        }
    }
